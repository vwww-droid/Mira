...
动态集成 busybox 命令集和 frida gadget, 交互式分析进程视角 procfs (iOS 为 syscall 模拟实现, 即使针对性优化, 执行 frida 命令仍有卡顿)
...
Mira 选择内置删减版 frida gadget 作为动态分析切点, 而非自定义 AOP, linker, xdl 库等生产方案. 后者其实会更强, 但实现一旦开源很容易被针对性研究和对抗, 反而把项目变成攻击者的训练样本.

这带来两个暂时性问题:
1. Frida 自身注入特征. 下个迭代会参考开源项目去内置 frida 特征, 同时继续识别外部 Frida 痕迹.
2. Frida 本身就容易崩. 短期需手动杀进程重启, 长期我会排查并逐步优化体验(欢迎大家提 issue 和 pr).


### 可选公网访问

配合 Relay, Mira 允许把一次临时授权会话使用 [脚本](./mira-web) 快速扩展到公网, 一是一些不便 adb 的场景(如云手机环境) 可以减少重复打包测试, 二是能快速请大佬协助分析 🤝
